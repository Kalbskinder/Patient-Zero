...
public class TeleportPlayers {
    ...
    private static final Logger LOGGER = Logger.getLogger("Infection");
    ...
    public void teleportPlayersOnGameStart(QueueInfo queue) {
        ...
        // Load survivor and infected spawns as single list
        locations.addAll(loadSpawnLocations(basePath + "survivor", "survivor"));
        locations.addAll(loadSpawnLocations(basePath + "infected", "infected"));

        // Check if there are spawn locations configured for the map
        // This can be done by using the in game command '/infection addspawn <map-name> <role>'
        if (locations.isEmpty()) {
            LOGGER.warning("No spawn locations available for map: " + map);
            queue.getPlayers().forEach(player -> { MMUtils.sendMessage(player, "<red>No spawn locations found! You can configure them by using '/infection addspawn <map-name> <role>'. For more information type '/infection help'"); });
            return;
        }
    ...
    // Teleports a player to a random defined infected spawn location
    public void teleportPlayerToInfectedLocations(Player player) {
        ...
        // Get the spawnpoints
        List<Location> locations = new ArrayList<>(loadSpawnLocations("maps." + mapName + ".spawns.infected", "infected"));
